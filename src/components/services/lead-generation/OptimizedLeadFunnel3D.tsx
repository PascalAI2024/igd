...
// 3D Funnel component with optimizations
const Funnel = React.memo(({ 
  ...
  useEffect(() => {
    ...
    // Cleanup function
    return () => {
      // Clear references to help prevent memory leaks
      lightRefs.current = [];
    };
  }, []);
  
  // Animation loop - with optimizations based on device capabilities
  useFrame((state, delta) => {
    const time = state.clock.getElapsedTime();
    
    // Calculate max particles based on performance level
    const maxParticles = performanceLevel === 'low' ? 50 : (performanceLevel === 'medium' ? 200 : 1000);
    
    // Add new particles at the top of the funnel - adjust rate based on performance
    if (Math.random() < (0.1 * particleMultiplier) && particles.current.length < maxParticles) {
    ...
    // Update particle attributes - optimize to update fewer particles on low-end devices
    const updateInterval = performanceLevel === 'low' ? 2 : 1;
    for (let i = 0; i < particles.current.length && i < 1000; i += updateInterval) {
      const idx = i * 3;
      const particle = particles.current[i];
    ...
    // Rotate funnel slightly to add visual interest - simplified on low-end devices
    if (funnelRef.current && performanceLevel !== 'low') {
    ...
    // Skip expensive light animations on low-end devices
    ...
    // Animate the point lights - throttle updates for medium-performance devices
    const updateFrequency = performanceLevel === 'medium' ? 2 : 1;
    lightRefs.current.forEach((lightGroup, index) => {
      if (index % updateFrequency !== 0) return; // Skip some lights on medium performance
  ...
  const pointsMaterial = useMemo(() => {
    ...
    return new THREE.ShaderMaterial({
      uniforms: {
        pointTexture: { value: textureLoader.load('/images/particle.png') }
      },
      vertexShader: `
  ...
  // Memoize stage interaction handlers
  const handleStageHover = useCallback((index: number) => {
    setHoveredStage(index);
  }, [setHoveredStage]);
  
  const handleStageHoverOut = useCallback(() => {
  ...
  return (
    <>
      ...
      {/* Animated lights under funnel - reduced for low performance */}
      <group position={[0, -3, -1]}>
        ...
        <spotLight 
          ...
          shadow-mapSize-width={performanceLevel === 'high' ? 1024 : 512}
          shadow-mapSize-height={performanceLevel === 'high' ? 1024 : 512}
          shadow-bias={-0.0001}
        />
        
        {/* Animated light particles - conditionally render based on performance level */}
        {stages.map((stage, index) => {
          // Skip some lights on low performance devices
          if (performanceLevel === 'low' && index % 2 !== 0) return null;
          ...
          return (
            ...
            >
              ...
              {/* Light ray effect - only on medium and high performance devices */}
        ...
        {/* Add volumetric light beams - only on high performance devices */}
      ...
      <mesh position={[0, -2.5, 0]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow={performanceLevel !== 'low'}>
        <planeGeometry args={[12, 6]} />
        <meshStandardMaterial 
          color="#111111"
          metalness={performanceLevel === 'low' ? 0.5 : 0.8}
          roughness={performanceLevel === 'low' ? 0.5 : 0.2}
          envMapIntensity={performanceLevel === 'low' ? 0.2 : 0.5}
        />
      </mesh>
      
      {/* Funnel shape */}
      <mesh ref={funnelRef} position={[0, 0, 0]} castShadow={performanceLevel !== 'low'}>
        <extrudeGeometry 
          args={[
            funnelShape, 
            { 
              depth: 0.5, 
              bevelEnabled: true,
              bevelSegments: performanceLevel === 'low' ? 1 : 2,
              steps: 1,
              bevelSize: 0.1,
              bevelThickness: 0.1
            }
        ...
        {stages.map((stage, index) => {
          ...
          return (
            <group 
              ...
              onPointerOver={() => handleStageHover(index)}
              onPointerOut={handleStageHoverOut}
            >
              <mesh>
...
const LeadFunnelFallback = React.memo(() => {
  return (
...
// Visibility detection hook with proper cleanup
...
const OptimizedLeadFunnel3D = React.memo(() => {
  ...
  const handleStageHoverEnd = useCallback(() => {
    setHoveredStage(null);
  }, []);
  ...
  return (
    <div className="relative min-h-[600px]" ref={containerRef}>
      ...
      {/* 3D Lead Funnel Visualization - only render when visible */}
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
        {stages.map((stage, index) => (
          <motion.div
            ...
            onHoverStart={() => handleStageHover(index)}
            onHoverEnd={handleStageHoverEnd}
            onClick={() => handleStageClick(index)}
...
OptimizedLeadFunnel3D.displayName = 'OptimizedLeadFunnel3D';

export default OptimizedLeadFunnel3D;